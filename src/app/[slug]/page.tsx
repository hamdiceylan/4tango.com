...
export default function PublicEventPage({ params }: { params: { slug: string } }) {
  ...
  // Check for reserved slugs immediately
  ...
  useEffect(() => {
    async function fetchEvent() {
      try {
        // Use preview endpoint if preview mode
        const endpoint = isPreview
          ? `/api/public/events/${params.slug}?preview=true`
          : `/api/public/events/${params.slug}`;
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">The event you&apos;re looking for doesn&apos;t exist or is not published.</p>
  ...
  // Get schedule data
  const schedule = (scheduleSection?.content?.days as Array<{
    label: string;
    day: string;
    date: string;
  ...
  // Get DJ team
  const djTeam = (djSection?.content?.members as Array<{
    name: string;
    country: string;
    photo: string;
  }>) || [];
  ...
  // Get photographers
  const photographers = (photographersSection?.content?.members as Array<{
    name: string;
    country: string;
    photo: string;
  }>) || [];
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative min-h-screen flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          {heroLogo && (
            <div className="mb-6">
              <img src={heroLogo} alt={event.title} className="h-24 md:h-32 w-auto mx-auto drop-shadow-lg" />
            </div>
          )}

          <h1 className="text-3xl md:text-5xl font-bold text-white mb-4 drop-shadow-lg" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>
            {event.title}
          </h1>

          <p className="text-white text-lg md:text-xl mb-4 drop-shadow-lg" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>
          ...
          {event.packages.length > 0 && (
            <p className="text-white text-lg md:text-xl font-medium mb-8 drop-shadow-lg" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>
              Starting from <span className="font-bold">€{Math.min(...event.packages.map(p => p.price)) / 100}</span> per person
            </p>
          )}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="bg-white border border-gray-200 p-6">
              <div className="text-center mb-4">
                <p className="font-semibold" style={{ color: primaryColor }}>{schedule[activeTab]?.day}</p>
              ...
              <div className="space-y-4">
                {schedule[activeTab]?.items.map((item, i) => (
                  <div key={i} className="flex justify-center gap-4 items-center py-2 border-b border-gray-100 last:border-0">
                    <span className="font-semibold w-32 text-right" style={{ color: primaryColor }}>{item.time}</span>
        ...
        <section id="accommodation" className="py-16 px-4 bg-white scroll-mt-16">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              ...
              {accommodation.rating && (
                <>
                  <div className="flex justify-center gap-0.5 mb-3">
                    {[1,2,3,4,5].map(i => (
                      <svg key={i} className="w-4 h-4" style={{ color: primaryColor }} fill="currentColor" viewBox="0 0 20 20">
              ...
              {accommodation.rating && (
                <p className="text-sm tracking-wider font-medium" style={{ color: primaryColor }}>{accommodation.rating}</p>
              )}
            </div>
            ...
            {accommodation.images && accommodation.images.length > 0 && (
              <div className="relative mb-10">
                  ...
                  >
                    {accommodation.images.map((img, i) => (
                      <div key={i} className="w-full flex-shrink-0">
                        <img src={img} alt={`${accommodation.name} ${i + 1}`} className="w-full h-80 object-cover" />
                      </div>
                    ))}
                  </div>
                </div>
                {accommodation.images.length > 1 && (
                  <>
                    <button
                      onClick={() => setHotelSlide((prev) => (prev - 1 + accommodation.images!.length) % accommodation.images!.length)}
                    ...
                    <button
                      onClick={() => setHotelSlide((prev) => (prev + 1) % accommodation.images!.length)}
            ...
            <div className="max-w-3xl mx-auto">
              ...
              {(accommodation.checkIn || accommodation.checkOut) && (
                <div className="flex justify-center gap-8 text-gray-600 text-sm">
                  {accommodation.checkIn && <p><strong>Check-in time is {accommodation.checkIn}</strong></p>}
                  {accommodation.checkOut && <p><strong>Check-out time is {accommodation.checkOut}</strong></p>}
                </div>
              )}
            </div>
          </div>
              ...
              {photographers.map((photographer, index) => (
                <div key={index} className="text-center" style={{ width: '180px' }}>
                  <div className="relative mx-auto w-44 h-44 mb-3">
                    <div className="absolute inset-0 rounded-full bg-gradient-to-br from-[#d4a853] via-[#f0d68a] to-[#b8962d] p-1.5">
                      <div className="w-full h-full rounded-full overflow-hidden bg-[#0a0a20] relative">
                        <img src={photographer.photo} alt={photographer.name} className="w-full h-full object-cover" />
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {event.packages.map((pkg) => (
                <div key={pkg.id} className="border border-gray-200 text-center p-6">
                  ...
                  {pkg.description && (
                    <>
                      <div className="w-8 h-px mx-auto mb-3" style={{ backgroundColor: primaryColor }}></div>
                      <p className="text-xs font-bold tracking-widest mb-3" style={{ color: primaryColor }}>{pkg.description}</p>
                    </>
                  )}
                  <div className="mb-4">
      ...
      <section id="register" className="py-16 px-4 bg-gray-50 scroll-mt-16">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-gray-600 mb-8">
            Ready to join us for an unforgettable tango experience? Click below to complete your registration.
          </p>
      ...
      <footer className="bg-[#0d0d1a] text-white py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          {heroLogo && (
            <img src={heroLogo} alt={event.title} className="h-20 w-auto mx-auto mb-4" />
          )}
          <p className="text-white/50 text-sm mb-6">
            {dateRange} • {event.city}, {event.country}
          </p>

          <a href={`mailto:${event.organizer.email}`} className="text-white/60 hover:text-[#d4a853] transition text-sm">
